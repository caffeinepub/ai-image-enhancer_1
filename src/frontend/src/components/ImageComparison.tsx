...
import { AlertCircle, ImageIcon, Layers, Sparkles, Upload } from "lucide-react";
...
import {
  ...
  type EnhancementValues,
  ImageEnhancementControls,
} from "./ImageEnhancementControls";
...
export function ImageComparison({
  ...
  // Trigger re-render of output canvas when baseCanvasRef changes
  const [baseCanvasVersion, setBaseCanvasVersion] = useState(0);

  const isUploading = isLoading && uploadProgress?.phase === "uploading";
  const isEnhancing =
    isLoading && (!uploadProgress || uploadProgress.phase === "enhancing");
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: baseCanvasVersion is a version counter for baseCanvasRef.current mutations
  useEffect(() => {
  ...
  }, [values, appliedCrop, baseCanvasVersion]);
  ...
  const showEnhanced =
    !isLoading && !error && enhancedUrl && !isProcessingCanvas;
  ...
  return (
    <div className="w-full animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Enhanced */}
        <div
          className={`card-gradient rounded-2xl overflow-hidden border transition-all duration-500 ${
            showEnhanced
              ? "border-primary/40 glow-amber-sm"
              : "border-charcoal-border"
          }`}
        >
          <div className="px-4 py-3 border-b border-charcoal-border flex items-center gap-2">
            <Sparkles
              className={`w-4 h-4 ${showEnhanced ? "text-primary" : "text-muted-foreground"}`}
            />
            <span
              className={`text-sm font-medium font-display ${showEnhanced ? "text-primary" : "text-muted-foreground"}`}
            >
              AI Enhanced
            </span>
            {showEnhanced && (
              ...
              >
                ...
                {crop.isActive && (
                  <div className="absolute inset-0 pointer-events-none">
                    ...
                    {/* Selection rectangle */}
                    {crop.displayRect &&
                      crop.displayRect.w > 0 &&
                      crop.displayRect.h > 0 && (
                        <div
